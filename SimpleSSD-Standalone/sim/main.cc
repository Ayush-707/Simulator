...
// Declaration
void cleanup(int);
std::vector<double> statistics(uint64_t);
void threadFunc(int);
...
int main(int argc, char *argv[]) {
  ...
  // if (tick != 0) {
  //   std::vector<double> statCopy = statistics(tick);
  // }
  // dataWriter.outputValues(statList, statCopy); // Call the outputValues function
...
void cleanup(int) {
  ...
  //dataWriter
  dataWriter.outputValues(statList, statCopy);
  // Cleanup all here
  delete pInterface;
  delete pIOGen;
...
std::vector<double> statistics(uint64_t tick) {
    if (pLog == nullptr) {
        std::cerr << "pLog is nullptr" << std::endl;
    ...
    // Assuming statList is a vector of Stats
    //std::vector<Stats> statList;
    // Assuming pInterface is available and getStats function exists
    ...
    if (count != stat.size()) {
        ...
        // Handle the error accordingly, maybe throw an exception or terminate the program
        std::terminate();
    }
    ...
    for (uint64_t i = 0; i < count; i++) {
        print(out, statList[i].name, 40); // Assuming print is a function for printing
        out << "\t";
        print(out, stat[i], 20); // Assuming print can print a double
